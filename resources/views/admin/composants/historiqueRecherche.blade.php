<div class="flex w-full h-full flex-col mb-4 font-barlow">
    <div class="w-full h-full flex flex-col justify-center items-center">
        <div
            class="grid grid-cols-1 xl:grid-cols-3 2xl:grid-cols-5 gap-3 h-full xl:h-full 2xl:h-5/6 w-5/6 bg-c2 overflow-y-auto">
            ...
            @else
                @foreach ($termesRecherche as $recherche)
                    <div class="flex w-full h-full justify-between items-center bg-c3 rounded-md p-1 px-3">
                        <h3 class="text-c1 font-barlow text-lg truncate text-ellipsis w-3/4">
                            {{ $recherche->termeRecherche }}</h3>
