<div class="w-full mb-5">
    ...
    @if (!$favorisLieux && !$favorisActivites)
        ...
        @if ($favorisLieux)
            ...
            <div dir="ltr">
                <div class="grid grid-flow-col auto-cols-max gap-x-4 overflow-x-auto snap-x whitespace-nowrap">
                    ...
                    @foreach ($favorisLieux as $lieuID)
                        @php
                            $lieu = \App\Models\Lieu::find($lieuID);
                        @endphp
                        @if ($lieu)
                            <a href="/lieu/zoom/{{ $lieu->id }}"
                                ...
                                <img src="{{ asset($lieu->photo_lieu_url) }}" alt="{{ $lieu->nomEtablissement }}"
                                ...
                                <div class="w-full flex justify-center items-end">
                                    <div class="opacity-90 bg-c1 flex w-full h-16 items-center">
                                        ...
                                        <div class="flex gap-x-2 px-4">
                                            <form action="{{ route('delete.favoris.lieu', ['id' => $lieu->id]) }}"
                                                method="POST">
                                                @csrf
        ...
        @if ($favorisActivites)
            ...
            <div dir="ltr">
                <div class="grid grid-flow-col auto-cols-max gap-x-4 overflow-x-auto snap-x whitespace-nowrap">
                    ...
                    @foreach ($favorisActivites as $activiteID)
                        @php
                            $activite = \App\Models\Activite::find($activiteID);
                        @endphp
                        @if ($activite)
                            <a href="/activite/zoom/{{ $activite->id }}/{{ $activite->lieux->first()->id ?? '' }}"
                                ...
                                <div class="w-full flex justify-center items-end">
                                    <div class="opacity-90 bg-c1 flex w-full h-16 items-center">
                                        ...
                                        <div class="ml-auto px-4 flex justify-end">
                                            <form
                                                action="{{ route('delete.favoris.activite', ['id' => $activite->id]) }}"
                                                method="POST">
                                                @csrf
