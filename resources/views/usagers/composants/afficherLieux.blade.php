<div id="afficherLieux">
    ...
    <div class="flex justify-end">
        <span class="uppercase text-base text-c3 font-semibold px-2.5 py-0.5 rounded-full bg-c5 inline-block w-20 text-center">
            {{__('inactif')}}
        </span></div>
    @else
    <div class="flex justify-end">
        <span class="uppercase text-base text-c3 font-semibold px-2.5 py-0.5 rounded-full bg-c1 inline-block w-20 text-center">
            {{__('actif')}}
        </span></div>
    @endif
    ...
    <div class="mb-4">
        ...
        <div class="text-base">
            ...
            <p><strong>{{ trans_choice('ville', 1) }} :</strong> {{ $lieu->ville()?->nom }}{{ $lieu->codePostal ? ', ' . $lieu->codePostal : '' }}</p>
            <p><strong>{{ __('pays') }} :</strong> {{ $lieu->pays()?->nom }}</p>
            <p><strong>{{ __('quartier') }} :</strong> {{ $lieu->quartier->nom }}</p>
            @if ($lieu->province())
            ...
            @endif
            @if ($lieu->siteWeb)
                <p class="truncate"><strong>{{ __('siteWeb') }} :</strong> <a href="{{ $lieu->siteWeb }}" class="text-blue-500 underline">{{ $lieu->siteWeb }}</a></p>
            @endif
            <p><strong>{{ __('telephone') }} :</strong> {{ $lieu->numeroTelephone }}</p>
            <p><strong>{{ __('typeLieu') }} :</strong> {{ $lieu->typeLieu->nom }}</p>
        </div>
    ...
    <div class="flex justify-end space-x-4 mt-auto">
        <button class="boutonSupprimer transform transition duration-300 hover:scale-110 text-[#B20101] hover:text-[#B50000]"
        ...
        <button class="boutonModifier transform transition duration-300 hover:scale-110 text-c1-500 hover:text-c1-700"
                data-lieuId="{{ $lieu->id }}" data-villeId="{{ $lieu->ville()?->id }}" data-typeLieuId="{{ $lieu->typeLieu->id }}">
