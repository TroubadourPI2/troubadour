        ...
        <div class="sm:hidden flex flex-row flex-wrap gap-4 items-center text-c1 rounded-lg">
            <div
                ...
                <div
                    ...
                    <h5 class="text-xl font-bold uppercase p-2 text-center flex items-center ">{{ $lieu->nomEtablissement }}</h5>
                </div>
                <div
                    ...
                    <div class="flex flex-col justify-between h-full">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="text-sm">
                                ...
                                <p><strong>Ville :</strong> {{ $lieu->ville()?->nom }}{{ $lieu->codePostal ? ', ' . $lieu->codePostal : '' }}</p>
                                ...
                                @endif
                                @if ($lieu->siteWeb)
                                <p><strong>Site Web :</strong> <a href="{{ $lieu->siteWeb }}" class="text-blue-500 underline">{{ $lieu->siteWeb }}</a></p>
                                @endif
                        ...
                        <div class="flex justify-end space-x-3 mt-3">
                            <button class="hover:text-red-500"><span class="iconify size-6" data-icon="ion:trash-outline" data-inline="false"></span></button>
                            <button class="modifierMobile hover:text-blue-500" data-lieuId="{{ $lieu->id }}"
                                data-villeId="{{ $lieu->ville()?->id }}"
                                data-typeLieuId="{{ $lieu->typeLieu->id }}"><span
                                    class="iconify size-6" data-icon="ep:edit"
            ...
            <div class="w-full sm:w-1/2 p-4 flex flex-col h-full gap-y-4">
                ...
                <div class="mb-4">
                    ...
                    <div class="text-base">
                        ...
                        @endif
                        @if ($lieu->siteWeb)
                        <p><strong>Site Web :</strong> <a href="{{ $lieu->siteWeb }}" class="text-blue-500 underline truncate">{{ $lieu->siteWeb }}</a></p>
                        @endif
                        <p><strong>Téléphone :</strong> {{ $lieu->numeroTelephone }}</p>
                        <p><strong>Type de lieu :</strong> {{ $lieu->typeLieu->nom }}</p>
                    </div>
                ...
                <div class="flex justify-end space-x-4 mt-auto">
                    <button class="transform transition duration-300 hover:scale-110 text-red-500 hover:text-red-700">
                        <span class="iconify size-8" data-icon="ion:trash-outline" data-inline="false"></span>
                    </button>
                    <button class="modifierWeb transform transition duration-300 hover:scale-110 text-c1-500 hover:text-c1-700" data-lieuId="{{ $lieu->id }}" data-villeId="{{ $lieu->ville()?->id }}" data-typeLieuId="{{ $lieu->typeLieu->id }}">
