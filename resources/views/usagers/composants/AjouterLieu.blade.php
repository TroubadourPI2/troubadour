    ...
    <form class="mt-2 text-c1" action="{{ route('usagerLieux.ajouterLieu') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="font-barlow text-c1 font-semibold mb-3">
            ...
            <div class="font-barlow text-c1 font-semibold uppercase mt-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-base sm:text-lg">
                    <div class="sm:col-span-1">
                        ...
                        <input type="text" name="nomEtablissement" id="nomEtablissement" class="block w-full rounded-lg p-1 sm:p-2 font-medium" value="{{ old('nomEtablissement') }}">
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('nomEtablissement'))
                        <span class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('nomEtablissement') }}</span>
                        @endif
                    </div>
                    <div class="sm:col-span-1">
                        <label for="selectTypeLieu" class="block">Type de lieu <span class="text-c5 ml-2">*</span></label>
                        <select name="selectTypeLieu" id="selectTypeLieu" class="block w-full rounded-lg p-2 sm:p-3 bg-c3 font-medium">
                            ...
                            <option value="{{ $type->id }}" {{ old('selectTypeLieu') == $type->id ? 'selected' : '' }}>
                                {{ $type->nom }}
                            </option>
                            @endforeach
                        </select>
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('selectTypeLieu'))
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('selectTypeLieu') }}</div>
                        @endif
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('description') }}</div>
                        @endif
                    </div>
                    <div class="sm:col-span-1">
                        ...
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('photoLieu') }}</div>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="font-barlow text-c1 font-semibold uppercase mt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-base sm:text-lg">
                    <div class="sm:col-span-1">
                        ...
                        <input type="text" name="noCivic" id="noCivic" class="block w-full rounded-lg p-1 sm:p-2 font-medium" value="{{ old('noCivic') }}">
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('noCivic'))
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('noCivic') }}</div>
                        @endif
                    </div>
                    <div class="sm:col-span-1">
                        <label for="rue" class="block">Rue <span class="text-c5 ml-2">*</span></label>
                        <input type="text" name="rue" id="rue" class="block w-full rounded-lg p-1 sm:p-2 font-medium" value="{{ old('rue') }}">
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('rue'))
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('rue') }}</div>
                        @endif
                    </div>
                    <div class="sm:col-span-1">
                        <label for="codePostal" class="block">Code postal <span class="text-c5 ml-2">*</span></label>
                        <input type="text" name="codePostal" id="codePostal" placeholder="A1A 1A1" class="block w-full rounded-lg p-1 sm:p-2 font-medium" value="{{ old('codePostal') }}">
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('codePostal'))
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('codePostal') }}</div>
                        @endif
                    </div>
                    <div class="sm:col-span-1">
                        ...
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('selectVilleLieu') }}</div>
                        @endif
                    </div>
                    <div class="sm:col-span-1">
                        <label for="selectQuartierLieu" class="block">Quartier (choisir une ville avant) <span class="text-c5 ml-2">*</span></label>
                        <select name="selectQuartierLieu" id="selectQuartierLieu" class="block w-full rounded-lg p-2 sm:p-3 bg-c3 font-medium" {{ old('selectVilleLieu') ? '' : 'disabled' }}>
                            <option value="">Sélectionner un quartier</option>
                        </select>
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('selectQuartierLieu'))
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('selectQuartierLieu') }}</div>
                        @endif
                    </div>
            ...
            <div class="font-barlow text-c1 font-semibold uppercase mt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-base sm:text-lg">
                    <div class="sm:col-span-1">
                        <label for="numeroTelephone" class="block">Numéro de téléphone <span class="text-c5 ml-2">*</span></label>
                        <input type="text" name="numeroTelephone" id="numeroTelephone" class="block w-full rounded-lg p-1 sm:p-2 font-medium" placeholder="###-###-####" value="{{ old('numeroTelephone') }}">
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('numeroTelephone'))
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('numeroTelephone') }}</div>
                        @endif
                    </div>
                    <div class="sm:col-span-1">
                        <label for="siteWeb" class="block">Site web</label>
                        <input type="text" name="siteWeb" id="siteWeb" class="block w-full rounded-lg p-1 sm:p-2 font-medium" placeholder="https://www.monsite.com/" value="{{ old('siteWeb') }}">
                        @if(session('erreurAjouterLieu') && session('erreurAjouterLieu')->has('siteWeb'))
                        <div class="text-c5 font-medium">{{ session('erreurAjouterLieu')->first('siteWeb') }}</div>
                        @endif
                    </div>
                </div>
            </div>
